...
public class WindmillStateInternalsTest {
  ...
  public void testMultimapGet() throws IOException {
    ...
    when(mockReader.multimapFetchSingleEntryFuture(
            encodeWithCoder(key, StringUtf8Coder.of()),
            key(NAMESPACE, tag),
            STATE_FAMILY,
            VarIntCoder.of()))
        .thenReturn(future);
    ...
    waitAndSet(future, Arrays.asList(1, 2, 3), 200);
  ...
  public void testMultimapRemoveAndGet() throws IOException {
    ...
    ReadableState<Iterable<Integer>> result = multimapState.get(key).readLater();
    ReadableState<Iterable<Integer>> result2 = multimapState.get(key).readLater();
    waitAndSet(future, Arrays.asList(1, 2, 3), 200);
    ...
    assertFalse(multimapState.containsKey(key).read());
  ...
  public void testMultimapGetLocalCombineStorage() throws IOException {
    ...
    ReadableState<Iterable<Integer>> result = multimapState.get(key).readLater();
    waitAndSet(future, Arrays.asList(1, 2), 200);
  ...
  @Test
  public void testMultimapLocalOverrideStorage() throws IOException {
    final String tag = "multimap";
    StateTag<MultimapState<String, Integer>> addr =
        StateTags.multimap(tag, StringUtf8Coder.of(), VarIntCoder.of());
    MultimapState<String, Integer> multimapState = underTest.state(NAMESPACE, addr);
  ...
  public void testMultimapLocalClearOverrideStorage() throws IOException {
    ...
    SettableFuture<Iterable<Integer>> future = SettableFuture.create();
    when(mockReader.multimapFetchSingleEntryFuture(
            encodeWithCoder(key1, StringUtf8Coder.of()),
            key(NAMESPACE, tag),
            STATE_FAMILY,
            VarIntCoder.of()))
        .thenReturn(future);
    ...
    when(mockReader.multimapFetchSingleEntryFuture(
            encodeWithCoder(key2, StringUtf8Coder.of()),
            key(NAMESPACE, tag),
            STATE_FAMILY,
            VarIntCoder.of()))
        .thenReturn(future2);
    ...
    ReadableState<Iterable<Integer>> result2 = multimapState.get(key2).readLater();
    multimapState.clear();
    waitAndSet(future, Arrays.asList(1, 2), 200);
  ...
  private static Map.Entry<ByteString, Iterable<Integer>> multimapEntry(
      String key, Integer... values) throws IOException {
    return new AbstractMap.SimpleEntry<>(
        encodeWithCoder(key, StringUtf8Coder.of()), Arrays.asList(values));
  }
  ...
  private static <K, V> void assertContainsEntry(
      Iterable<Map.Entry<K, Iterable<V>>> entries, K key, V... values) {
    assertThat(
        entries,
        Matchers.hasItem(
            Matchers.both(Matchers.hasProperty("key", Matchers.equalTo(key)))
                .and(Matchers.hasProperty("value", Matchers.containsInAnyOrder(values)))));
  }
  ...
  public void testMultimapBasicEntriesAndKeys() throws IOException {
    ...
    ReadableState<Iterable<Map.Entry<String, Iterable<Integer>>>> entriesResult =
        multimapState.entries().readLater();
    ReadableState<Iterable<String>> keysResult = multimapState.keys().readLater();
    waitAndSet(
        entriesFuture,
    ...
    Iterable<Map.Entry<String, Iterable<Integer>>> entries = entriesResult.read();
    assertEquals(2, Iterables.size(entries));
    assertContainsEntry(entries, key1, 1, 2, 3);
    assertContainsEntry(entries, key2, 4, 2, 3);

    Iterable<String> keys = keysResult.read();
  ...
  public void testMultimapEntriesAndKeysMergeLocalAdd() throws IOException {
    ...
    multimapState.put(key3, 8);

    Iterable<Map.Entry<String, Iterable<Integer>>> entries = entriesResult.read();
    assertEquals(3, Iterables.size(entries));
    assertContainsEntry(entries, key1, 1, 2, 3, 7);
    assertContainsEntry(entries, key2, 4, 2, 3, 8);
    assertContainsEntry(entries, key3, 8);
  ...
  public void testMultimapEntriesPaginated() throws IOException {
    ...
    Iterable<Map.Entry<String, Iterable<Integer>>> entries = entriesResult.read();
    assertEquals(2, Iterables.size(entries));
    assertContainsEntry(entries, key2, 2, 3, 4, 4, 5, 8);
    assertContainsEntry(entries, key3, 8);
  ...
  public void testMultimapCacheComplete() throws IOException {
    ...
    waitAndSet(entriesFuture, weightedList(multimapEntry(key, 1, 2, 3)), 200);
    ...
    Iterable<Map.Entry<String, Iterable<Integer>>> entries = multimapState.entries().read();
    assertEquals(1, Iterables.size(entries));
    assertContainsEntry(entries, key, 1, 2, 3, 2);

    Iterable<String> keys = multimapState.keys().read();
    assertThat(keys, Matchers.containsInAnyOrder(key));

    Iterable<Integer> values = multimapState.get(key).read();
  ...
  public void testMultimapCachedSingleEntry() throws IOException {
    ...
    waitAndSet(entryFuture, weightedList(1, 2, 3), 200);
    multimapState.get(key).read();
    multimapState.put(key, 2);

    when(mockReader.multimapFetchSingleEntryFuture(
            eq(encodeWithCoder(key, StringUtf8Coder.of())),
  ...
  public void testMultimapCachedPartialEntry() throws IOException {
    ...
    assertEquals(3, Iterables.size(entries));
    assertContainsEntry(entries, key1, 1, 2, 2, 3);
    assertContainsEntry(entries, key2, 4, 5, 6);
    assertContainsEntry(entries, key3, 20);
  ...
  private static MultimapEntryUpdate decodeTagMultimapEntry(Windmill.TagMultimapEntry entryProto) {
    try {
      String key = StringUtf8Coder.of().decode(entryProto.getEntryName().newInput());
      List<Integer> values = new ArrayList<>();
      for (ByteString value : entryProto.getValuesList()) {
        values.add(VarIntCoder.of().decode(value.newInput()));
      }
  ...
  @Test
  public void testMultimapClearPutRemoveAndPersist() {
    final String tag = "multimap";
    StateTag<MultimapState<String, Integer>> addr =
        StateTags.multimap(tag, StringUtf8Coder.of(), VarIntCoder.of());
    MultimapState<String, Integer> multimapState = underTest.state(NAMESPACE, addr);
    ...
    // put after clear should be sent
    multimapState.put(key1, 4);

    // put then remove should not be sent
